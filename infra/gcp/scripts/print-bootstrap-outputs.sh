...
echo ""
echo "Set GITHUB_DEPLOY_TARGET=gcp on the deploying GitHub repo if you want image updates from CI."
echo "Local ./deploy.sh still builds and pushes even when GitHub is configured."
echo "(gcloud run services update --image); terraform apply ignores the container image."
echo ""
echo "DATA_ENCRYPTION_KEY comes from the .env.deploy file, or from Secret Manager"
echo "when GCP_USE_SECRET_MANAGER is true. Back it up securely (see docs/DEPLOY.md)."
