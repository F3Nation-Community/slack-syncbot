...
8. **Plan files** — do not commit `docs/PLAN_*.md`, `PLAN_*.md`, or `.plans/`. Keep them local; they are gitignored.
...
- **User Mapping is a modal.** Open from DB mappings only (`views.open` on `trigger_id`); never seed/match/crawl before open. **Auto Map Now** updates the open modal via `view_id` (Mapping users... → directory map → last-run line). Never `views.publish` Home for mapping. Auto Map Now from `user_directory.email` with `allow_slack_email_lookup=False` on the button (no source `users.info` either); do not crawl `users.list` on open, Refresh List, Auto Map Now, or group join. **On-the-fly author map** (`ensure_mapped_target_user_id`) is one person, email only (directory then one dest `lookupByEmail`); not a full Auto Map Now.
