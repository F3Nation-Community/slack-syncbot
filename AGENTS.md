...
- **Direct reactions** use `get_user_token(dest_team_id, mapped_user_id)`, never the event team. Never send `xoxp` on federation wire payloads.
...
- **Hybrid thread.** Dest `invalid_name` never threads. Probe dest emoji names with the bot token **only** when about to post a Hybrid thread (no dest `xoxp`, or that token hit `_NO_AUTHORIZE_ERRORS`). Direct-only and a successful native add never probe. Do not use `emoji.list`.
