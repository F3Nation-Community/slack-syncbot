...
- **Direct reactions** use `get_user_token(dest_team_id, mapped_user_id)`, never the event team. Never send `xoxp` on federation wire payloads. Native `reaction_added` is the mapped person, not the bot. After a user-token `reactions_add` / `reactions_remove`, call `remember_user_action`; call `take_user_action_echo` **inside** `run_claimed` before fan-out (not `processed_events`). Probe dest emoji names once per workspace (bot token) at the start of apply; Hybrid threads only when that probe says the name exists and there is no dest token or an auth error. `invalid_name` and `already_reacted` never thread.
