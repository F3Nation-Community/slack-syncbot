...
| `DATABASE_USER` | Username. Required when backend is `mysql` or `postgresql` and `DATABASE_URL` is unset. Some providers (e.g. TiDB Cloud Serverless) require a cluster-specific prefix on every SQL user; set **`DATABASE_USERNAME_PREFIX`** so the bootstrap Lambda prepends it to **`DATABASE_ADMIN_USER`** and to the default app user `{prefix}.sbapp_{stage}` (a dot separator is added automatically; use bare admin names like `root` when set). **`DATABASE_APP_USERNAME`** optionally sets the full app username and bypasses prefix + default. Deploy scripts map these env vars to the underlying SAM parameters (`ExistingDatabaseUsernamePrefix`, etc.) and Terraform variables (`existing_db_username_prefix`, etc.) automatically. New stack-managed RDS uses master **`sbadmin_{stage}`** and app **`sbapp_{stage}`**. |
...
**SQLite (local / GCP default):** Set `DATABASE_BACKEND=sqlite` and `DATABASE_URL=sqlite:///path/to/file.db`. Single-writer; suitable for small teams and dev. **GCP Cloud Run** injects `DATABASE_URL=sqlite:////data/syncbot.db` (four slashes = absolute `/data/syncbot.db`) so the file matches Litestream restore/replicate. `LITESTREAM_GCS_BUCKET` is container/infra-only — the app does not read it. Horizontal scaling is not supported with SQLite (`max_instances=1` on that provider).
...
**PostgreSQL:** Set `DATABASE_BACKEND=postgresql` and either `DATABASE_URL` (`postgresql+psycopg2://...`) or `DATABASE_HOST`, `DATABASE_USER`, `DATABASE_PASSWORD`, `DATABASE_SCHEMA`. Deploy-time bootstrap credentials (`DATABASE_ADMIN_USER` / `DATABASE_ADMIN_PASSWORD`) are used only for one-time setup; the app reads `DATABASE_USER`, `DATABASE_PASSWORD`, `DATABASE_SCHEMA` at runtime.
...
**Reference wiring:** AWS SAM ([`infra/aws/template.yaml`](../infra/aws/template.yaml)) maps CloudFormation parameters to Lambda env: **`SlackOauthBotScopes`** / **`SlackOauthUserScopes`** → **`SLACK_BOT_SCOPES`** / **`SLACK_USER_SCOPES`** (defaults match `BOT_SCOPES` / `USER_SCOPES`); **`LogLevel`** → **`LOG_LEVEL`**; **`RequireAdmin`** → **`REQUIRE_ADMIN`**; **`SoftDeleteRetentionDays`** → **`SOFT_DELETE_RETENTION_DAYS`**; **`SyncbotFederationEnabled`**, **`SyncbotInstanceId`**, **`SyncbotPublicUrl`** (optional override) → federation env vars; **`PrimaryWorkspace`** → **`PRIMARY_WORKSPACE`**; **`EnableDbReset`** → **`ENABLE_DB_RESET`** (boolean `true` when enabled); optional **`DatabaseTlsEnabled`** / **`DatabaseSslCaPath`** → **`DATABASE_TLS_ENABLED`** / **`DATABASE_SSL_CA_PATH`** (omit when empty so app defaults apply). When using an **existing** DB host: optional **`ExistingDatabasePort`** → **`DATABASE_PORT`** (empty uses engine default); **`ExistingDatabaseCreateAppUser`** / **`ExistingDatabaseCreateSchema`** control the DB setup custom resource (dedicated app user and `CREATE DATABASE`), not direct Lambda env names—see [DEPLOY.md](DEPLOY.md). Deploy scripts and CI workflows map simplified env vars (e.g. `DATABASE_HOST`, `DATABASE_ADMIN_USER`) to these SAM parameters (`ExistingDatabaseHost`, `ExistingDatabaseAdminUser`, etc.) automatically; old `EXISTING_DATABASE_*` env var names are still accepted as fallbacks. **`SYNCBOT_PUBLIC_URL`** defaults to the Lambda Function URL unless **`SyncbotPublicUrl`** is set; stack output **`SyncBotPublicBaseUrl`** documents that base. GCP Terraform ([`infra/gcp`](../infra/gcp/README.md)) uses **`database_mode`**: `sqlite` (default) injects `DATABASE_BACKEND=sqlite`, `DATABASE_URL=sqlite:////data/syncbot.db`, and `LITESTREAM_GCS_BUCKET`; `existing` injects host/user/password/schema like AWS existing-host (TiDB port **4000** / username prefix supported). Cloud SQL is not created. Variables **`slack_bot_scopes`** → `SLACK_BOT_SCOPES` and **`slack_user_scopes`**, **`log_level`**, **`require_admin`**, **`database_backend`**, **`database_port`**, **`soft_delete_retention_days`**, **`syncbot_federation_enabled`**, **`syncbot_instance_id`**, **`syncbot_public_url_override`**, **`primary_workspace`**, **`enable_db_reset`**, **`database_tls_enabled`**, **`database_ssl_ca_path`** map to Cloud Run env. When **`database_mode=existing`**, **`existing_db_create_app_user`** / **`existing_db_create_schema`** are recorded as Cloud Run service labels for operator documentation. **`syncbot_public_url_override`** is empty by default—set it to your service’s public HTTPS base (e.g. after first deploy) if you need **`SYNCBOT_PUBLIC_URL`** for federation.

Deploy-only warmth knobs are **not** app runtime env: **`GCP_CLOUD_RUN_MIN_INSTANCES`** (`0` or `1`) and **`ENABLE_KEEP_WARM`** (portable name; GCP Terraform `enable_keep_warm` only in this repo until AWS is wired). Do not inject them into the Slack process.
...
3. **Database**
   **PostgreSQL / MySQL:** In non–local environments the app uses TLS by default; allow outbound TCP to the DB host (typically **5432** for PostgreSQL, **3306** for MySQL, **4000** for TiDB Cloud). **SQLite:** No network; the app uses a local file. Single-writer; ensure backups and file durability for production use (GCP reference: Litestream replica in GCS). Cloud SQL is not required.
