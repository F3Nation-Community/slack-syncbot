...
- **@mentions and #channel links** in synced messages are rewritten for the destination: mapped users are tagged with the local Slack user, and `#channel` mentions become a code-ticked `#name (Workspace)` (for example `` `#ao-bridge-to-nowhere (F3 T-Town Test)` ``). SyncBot does not turn those into the destination twin channel, because the author named a place in the source workspace. Links to a specific source message keep a labeled link back to that message in the source workspace. Those links open the source message in the Slack mobile app; in Slack’s desktop browser they may appear as a private message in the destination workspace. Unmapped people fall back to a code-ticked display name such as `` `Name (Workspace)` ``.
